...
jQuery(window).load(function($) {
  jQuery('#advisories-placeholder').load('ajax.advisories.php?loc=<?= LOCATION ?>');
  jQuery('#alerts-placeholder').load('ajax.alerts.php?loc=<?= LOCATION ?>');
  jQuery('#twitter-placeholder').load('ajax.twitter.php?loc=<?= LOCATION ?>', function(response, status, xhr) {
    if (status != "error") {
  ...
  jQuery('#cameras-placeholder').load('ajax.cameras.php?loc=<?= LOCATION ?>', function(response, status, xhr) {
    if (status != "error") {
  ...
  jQuery('#streams-placeholder').load('ajax.streams.php?loc=<?= LOCATION ?>');
  <?php endif ?>
  jQuery('#resources-placeholder').load('ajax.resources.php?loc=<?= LOCATION ?>');
  jQuery('#hydrograph-placeholder').load('ajax.hydrograph.php?loc=<?= LOCATION ?>');
});
...
function updateCameras() {
  jQuery('#camera-redriver-fargo').attr('src', 'http://nd.water.usgs.gov/FARGOwebcam/webcama.JPG?' + new Date());
  jQuery('#camera-redriver-fargo').fadeIn('slow');
  jQuery('#camera-cityoffargo-webcam1').attr('src', 'http://webcam1.cityoffargo.com/jpg/image.jpg?' + new Date());
  jQuery('#camera-cityoffargo-webcam1').fadeIn('slow');
